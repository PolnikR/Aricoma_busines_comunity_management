...
interface IbmPowerDetailPanelProps {
  ...
  open: boolean
  onClose: () => void
  labels: Record<PowerFieldGroup | 'selected' | 'detail' | 'close' | 'summary' | 'provider' | 'partitionKind' | 'partitionState' | 'interfaceState' | 'ipAddress' | 'subnetMask' | 'isBootable' | 'maximumVirtualIoSlots', string>
}
        ...
        <div className="space-y-5 p-5">
          <section>
            ...
            <dl>
              ...
              <DetailRow label={labels.interfaceState} value={display(partition.partitionData['State'])} />
              <DetailRow label={labels.ipAddress} value={display(partition.partitionData['IPAddress'])} />
              <DetailRow label={labels.subnetMask} value={display(partition.partitionData['SubnetMask'])} />
              <DetailRow label={labels.isBootable} value={display(partition.partitionData['IsBootable'])} />
              <DetailRow label={labels.maximumVirtualIoSlots} value={display(partition.partitionData['MaximumVirtualIOSlots'])} />
            </dl>
          </section>
          {groupOrder.map((group) => {
            ...
            return (
              <section key={group}>
                ...
                <dl>{fields.map((field) => <DetailRow key={field.key} label={field.label} value={display(partition.partitionData[field.key])} />)}</dl>
              </section>
            )
          })}
        </div>
